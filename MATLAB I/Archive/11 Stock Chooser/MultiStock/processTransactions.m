...
% Create transaction file
    ...
    for tn = 1:numel(transactions.Timestamp)
        ...
        try
            ...
            fprintf(xfileID, "%s is not in the price list\n", xstock )
            continue
        end
        ...
        fprintf(xfileID, "TRAN:%s: %s wants to %s %.3f shares of %s for $%.2f total\n", xdate, xplayer, xtype, xshares, string(xstock), xcash);
...
% Now find the column for the stock in the stocks array.
        ...
        if ~stockCol
            fprintf(xfileID, "ERROR: %s does not exist in stock list\n", xstock)
            continue
        end
...
% player's cash to |cash|.
            ...
            if xshares > holding
                fprintf(xfileID, "ERROR: %s has %.2f shares of %s and wants to sell %.2f\n", ...
