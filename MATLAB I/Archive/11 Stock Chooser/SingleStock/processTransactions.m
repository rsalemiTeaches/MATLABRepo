...
% We read all the stock price history for the stocks in the stock list from 
% the beginning of the year to today.  We use this data to determine profit, loss, 
% and final value of each stock.

    % Get the prices from Yahoo if prices doesn't exist.  No sense rereading
    % them.
    prices = hist_stock_data('01012023',now,stockFile);
...
% data.
    ...
    for tn = 1:numel(transactions.Timestamp)
        ...
        try
            stockStruct = prices({prices.Ticker} == xstock); % Get the table row for the stock
        catch
...
% array.
        ...
        if xtype == "Buy"
            if playercash < xcash
                fprintf("%s has %.2f dollars and needs %.2f to buy %.2f shares of %s\n", string(xplayer), playercash, xcash, xshares, xstock);
                continue;
            end
...
% of the holdings using the most recent |adjClose| prices. 
    ...
    sortrows(finalTable,"CashValue", "descend");  % Sort the table before returning it.
end
%% 
%